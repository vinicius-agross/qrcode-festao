...
def gerar_pdf_qrcodes(request):
    ...
    for (cnpj, razsoc), convidados in empresas.items():
        ...
        for convidado in convidados:
            ...
            if y < 80 * mm:
                _nova_pagina(c, width, razsoc, cnpj)
                y = height - 100
                x = 30 * mm
                count = 0

        _nova_pagina(c, width)
...
def _nova_pagina(c, width, razsoc=None, cnpj=None):
    ...
    if razsoc and cnpj:
        c.setFont("Helvetica-Bold", 20)
        c.setFillColorRGB(0, 0, 0)
        c.drawCentredString(width / 2, A4[1] - 50, f"{razsoc} ({cnpj})")


@login_required
def gerador(request):
    cnpj = request.user.username
    convidados = Cliente.objects.filter(cnpj=cnpj) if cnpj else []
...
def cadastro(request):
    if request.method == 'GET':
        return _cadastro_get(request)
    elif request.method == 'POST' and 'cnpj_final' in request.POST:
        return _cadastro_post(request)
    return render(request, 'pages/cadastro.html')


def _cadastro_get(request):
    cnpj = request.user.username
    try:
...
def _cadastro_post(request):
    ...
    nome_responsavel = request.POST.get('nome_responsavel_final', '').strip()
    convidados = []
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if cliente.email:
                ...
                cache.set(token, cnpj, timeout=3600)
...
def cadastrar_senha(request):
    ...
    return render(request, 'cadastro/cadastrar_confirm.html', {'cnpj': cnpj, 'message': message})
